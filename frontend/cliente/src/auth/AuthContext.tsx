...
import type { AuthResponse, RegisterClienteRequest, User } from '@/types/api'
...
type AuthContextValue = State & {
  ...
  registerCliente: (input: RegisterClienteRequest) => Promise<void>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const registerCliente = useCallback(
    async (input: RegisterClienteRequest) => {
      const res = await apiClient.post<AuthResponse>('/auth/register/cliente', input)
      authenticate(res)
    },
    [authenticate],
  )
  ...
  const setUser = useCallback((user: User) => {
    const saved = authStorage.load()
    if (saved) authStorage.save({ ...saved, user })
  ...
  const value = useMemo<AuthContextValue>(
    () => ({ ...state, login, registerCliente, setUser, logout }),
    [state, login, registerCliente, setUser, logout],
  )
